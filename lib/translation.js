...
/**
 * Extract key terms from text for action plan generation
 */
...
/**
 * Generate action plan based on document content (no API needed)
 */
async function generateActionPlan(summary, targetLanguage, fullText = '') {
  try {
    ...
    // Generate context-aware action plan based on document type
    let actionPlan = [];
    ...
    // Translate action plan if not English
    ...
    // Return basic fallback action plan
    return [
      {
